...
public class Robot implements Runnable, PoseProvider, MoveListener {
	...
	public lejos.robotics.navigation.Pose getPose() {
		Pose p = pose;
		if(motors.isMoving()) {
			Move m = motors.getMovement();
			p = new Pose(pose.getX(), pose.getY(), pose.getHeading());
			p.updatePose(m);
		}
		return p;
	}
